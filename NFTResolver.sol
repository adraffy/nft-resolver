...
import {IReverseRegistrar} from "https://github.com/ensdomains/ens-contracts/blob/master/contracts/reverseRegistrar/IReverseRegistrar.sol";
...
bytes32 constant ENS_REVERSE_NODE = 0x91d1777781884d03a6757a803996e38de2a42967fb37eeaca72729271025a9e2; // namehash("addr.reverse")

uint256 constant HYPHEN = 0x2D;
	
contract NFTResolver is Ownable, IERC165, IExtendedResolver {
	...
	function setPrimary(string calldata name) onlyOwner external {
		IReverseRegistrar(ENS(ENS_REGISTRY).owner(ENS_REVERSE_NODE)).setName(name);
	}
	...
	function holderOf(address nft, uint256 token) public view returns (address target, address owner) {
		if (nft == CRYPTO_PUNKS) {
			...
			try IERC721(nft).ownerOf(token) returns (address ret) {
				owner = ret;
			} catch {
			}
		}
		...
		while (owner.code.length > 0) { // wrapped?
			try IERC721(owner).ownerOf(token) returns (address ret) {
				target = owner;
				owner = ret;
			} catch {
				break;
		...
		unchecked {
			...
			if (method == 0x3b3b57de) { // addr(bytes32) => address
				...
				if (coinType == 60) {
					if (ty == TY_TOKEN) {
						(, addr) = holderOf(addr, token);
					} else if (addr == address(0) && ENS(ENS_REGISTRY).resolver(node) == address(this)) {
						addr = address(this);
					}
				...
				if (keyhash == 0xb68b5f5089998f2978a1dcc681e8ef27962b90d5c26c4c0b9c1945814ffa5ef0) { // url
					if (ty == TY_TOKEN) {
						try IERC721Metadata(addr).tokenURI(token) returns (string memory url) {
							return abi.encode(url);
						} catch {
						}
					} 
				} else if (keyhash == 0x1596dc38e2ac5a6ddc5e019af4adcc1e017a04f510d57e69d6879d5d2996bb8e) { // description
					try IERC721Metadata(addr).name() returns (string memory ret) {
